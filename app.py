...
def upsert_document():
    ...
    docs = db.collection(collection_name).where('name', '==', document.name).stream()
    doc_ids = [doc.id for doc in docs]
    if not doc_ids:
        db.collection(collection_name).add(document.dict())
        return jsonify({"success": True, "message": "Document created"}), 201
    else:
        for doc_id in doc_ids:
            db.collection(collection_name).document(doc_id).update(document.dict())
